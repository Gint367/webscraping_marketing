...
def extract_and_log_domains(
    df, url_column_name, column_to_create="base_domain", sample_size=5, description=""
):
    """
    ...
    if url_column_name in df.columns:
        ...
        for url in sample_urls:
            logger.debug(
                f"  - URL: {url}, Extracted domain: {extract_base_domain(url)}"
            )
        ...
        logger.warning(
            f"Column '{url_column_name}' not found in DataFrame {description}"
        )
...
def merge_csv_with_excel(csv_path, base_data_path, output_path=None):
    ...
    for standard_name, possible_names in BASE_COLUMN_MAPPING.items():
        ...
        if actual_name:
            ...
            logger.warning(
                f"Could not find a column matching '{standard_name}' (tried: {', '.join(possible_names)})"
            )

    # Check if we found all required columns
    if (
        len(actual_base_columns) < 4
    ):  # At minimum need company name, location, technical equipment, and URL
        missing_columns = set(BASE_COLUMNS[:4]) - set(column_mapping.values())
        logger.error(
            f"Not enough columns found in base data. Missing essential columns: {missing_columns}"
        )
        raise ValueError(
            f"Not enough columns found in base data. Missing essential columns: {missing_columns}"
        )
    ...
    if len(available_base_columns) < len(BASE_COLUMNS):
        logger.warning(
            f"Some standard columns are missing from base data: {set(BASE_COLUMNS) - set(available_base_columns)}"
        )
    ...
    if "Top1_Machine" in filtered_data.columns:
        filtered_data = filtered_data.rename(
            columns={"Top1_Machine": "technische Anlagen und Maschinen 2021/22"}
        )
    else:
        # Add an empty column if Top1_Machine doesn't exist
        filtered_data["technische Anlagen und Maschinen 2021/22"] = None
        logger.warning(
            "Column 'Top1_Machine' not found in base data, added empty column"
        )
    ...
    if "Firma1" in filtered_data.columns:
        ...
        if not changed_rows.empty:
            logger.info(
                f"Cleaned trailing symbols in {len(changed_rows)} company names:"
            )
    ...
    if url_column:
        csv_data = extract_and_log_domains(
            csv_data, url_column, description="from CSV data"
        )
    else:
        logger.warning(
            "No URL column found in CSV data. Tried: Website, Company URL, Company Url, URL, website, url"
        )
        csv_data["base_domain"] = None

    # Extract domains from filtered data
    filtered_data = extract_and_log_domains(
        filtered_data, "URL", description="from base data"
    )
    ...
    if FUZZY_AVAILABLE:
        ...
        for idx, row in csv_data.iterrows():
            ...
            logger.info(
                f"PROGRESS:merge_pipeline:merge_technische_anlagen:{current_company_num}/{total_companies}:Attempting name match for {row.get('Company name', 'Unknown')}"
            )

            if pd.notna(company_name):
                # Try exact matching first
                exact_matches = filtered_data[
                    filtered_data["firma1_lower"] == company_name
                ]
                ...
                if not exact_matches.empty:
                    ...
                    if matches:
                        ...
                        if len(best_match) >= 2:
                            ...
                            match_idx = filtered_data[
                                filtered_data["firma1_lower"] == match_text
                            ].index[0]
                            matched_data = filtered_data.loc[match_idx]
                            match_stats["fuzzy_name_matches"] += 1

                            logger.debug(
                                f"Fuzzy match found for '{row['Company name']}' → '{matched_data['Firma1']}' (score: {match_score}%)"
                            )

                            # If there are multiple good matches, log them
                            if len(matches) > 1:
                                logger.debug(
                                    f"  Other potential matches for '{row['Company name']}':"
                                )
                                for i, match_data in enumerate(matches[1:], 1):
                                    # Safely handle different tuple lengths
                                    match_text = match_data[0]
                                    match_score = (
                                        match_data[1] if len(match_data) > 1 else "N/A"
                                    )
                                    logger.debug(
                                        f"    {i}. '{match_text}' (score: {match_score}%)"
                                    )

            # If we found a match (exact or fuzzy), copy the data
            if matched_data is not None:
                for col in [
                    "technische Anlagen und Maschinen 2021/22",
                    "Ort",
                    "URL",
                    "Sachanlagen",
                    "Firma1",
                ...
                if "Maschinen_Park_Size" in matched_data:
                    if pd.isna(matched_data["Maschinen_Park_Size"]):
                        ...
                        merged_data.at[idx, "Maschinen_Park_Size"] = str(
                            matched_data["Maschinen_Park_Size"]
                        )

                # Log if both technische Anlagen und Maschinen and Sachanlagen are empty
                tech_value = matched_data.get(
                    "technische Anlagen und Maschinen 2021/22", None
                )
                sach_value = matched_data.get("Sachanlagen", None)
                if (pd.isna(tech_value) or tech_value == "" or tech_value is None) and (
                    pd.isna(sach_value) or sach_value == "" or sach_value is None
                ):
                    company_name = matched_data.get("Firma1", "N/A")
                    logger.warning(
                        f"Technical data empty for matched company: '{company_name}'"
                    )

    name_matched = match_stats["exact_name_matches"] + match_stats["fuzzy_name_matches"]
    logger.info(
        f"Matched {name_matched} companies by name (Exact: {match_stats['exact_name_matches']}, Fuzzy: {match_stats['fuzzy_name_matches']})"
    )
    ...
    if unmatched_count > 0:
        ...
        for idx, row in unmatched_data.iterrows():
            ...
            domain = (
                row["base_domain"]
                if "base_domain" in row and pd.notna(row["base_domain"])
                else row.get("base_domain_x")
                if "base_domain_x" in row
                else "N/A"
            )
            logger.debug(
                f"  - Unmatched: Company: '{company_name}', URL: '{url}', Domain: '{domain}'"
            )
        ...
        for idx, row in unmatched_data.iterrows():
            ...
            logger.info(
                f"PROGRESS:merge_pipeline:merge_technische_anlagen:{idx + 1}/{len(csv_data)}:Attempting URL match for {row.get('Company name', 'Unknown')} (Domain: {csv_domain})"
            )
            ...
            if pd.notna(csv_domain) and csv_domain and csv_domain not in used_domains:
                ...
                if csv_domain in domain_lookup:
                    ...
                    if len(match_indices) > 0:  # Found match(es)
                        if len(match_indices) > 1:
                            ...
                            company_names = [
                                filtered_data.loc[i, "Firma1"] for i in match_indices
                            ]
                            logger.warning(
                                f"Multiple matches found for domain '{csv_domain}':"
                            )
                            for i, company in enumerate(company_names):
                                logger.warning(
                                    f"  - {company} (from {filtered_data.loc[match_indices[i], 'Ort']})"
                                )
                            logger.warning(
                                f"  Taking the first match: '{company_names[0]}'"
                            )
                ...
                if FUZZY_AVAILABLE:
                    ...
                    available_domains = [
                        d for d in domain_lookup.keys() if d not in used_domains
                    ]
                    if available_domains:
                        ...
                        if fuzzy_matches:
                            ...
                            if len(match_indices) > 1:
                                match_stats["duplicate_domains"] += 1
                                company_names = [
                                    filtered_data.loc[i, "Firma1"]
                                    for i in match_indices
                                ]
                                logger.warning(
                                    f"Multiple matches for fuzzy domain '{best_domain}' (score: {score}%):"
                                )
                                for i, company in enumerate(company_names):
                                    logger.warning(
                                        f"  - {company} (from {filtered_data.loc[match_indices[i], 'Ort']})"
        ...
        url_matches = (
            match_stats["exact_url_matches"] + match_stats["fuzzy_url_matches"]
        )
        logger.info(
            f"Additionally matched {url_matches} companies by URL (Exact: {match_stats['exact_url_matches']}, Fuzzy: {match_stats['fuzzy_url_matches']})"
        )
        if match_stats["duplicate_domains"] > 0:
            logger.warning(
                f"Found {match_stats['duplicate_domains']} domains with multiple company matches"
            )
    else:
        logger.info(
            "All companies were successfully matched by name. Skipping URL matching."
        )
        url_matches = 0
    ...
    merged_data = merged_data.drop(
        [col for col in drop_cols if col in merged_data.columns], axis=1
    )
    ...
    logger.info(
        f"  - Name matches: {name_matched} ({name_matched / total_csv_rows * 100:.1f}%)"
    )
    logger.info(
        f"    - Exact name matches: {match_stats['exact_name_matches']} ({match_stats['exact_name_matches'] / total_csv_rows * 100:.1f}%)"
    )
    logger.info(
        f"    - Fuzzy name matches: {match_stats['fuzzy_name_matches']} ({match_stats['fuzzy_name_matches'] / total_csv_rows * 100:.1f}%)"
    )
    logger.info(
        f"  - URL matches: {url_matches} ({url_matches / total_csv_rows * 100:.1f}%)"
    )
    logger.info(
        f"    - Exact URL matches: {match_stats['exact_url_matches']} ({match_stats['exact_url_matches'] / total_csv_rows * 100:.1f}%)"
    )
    logger.info(
        f"    - Fuzzy URL matches: {match_stats['fuzzy_url_matches']} ({match_stats['fuzzy_url_matches'] / total_csv_rows * 100:.1f}%)"
    )
    logger.info(
        f"Unmatched: {match_stats['unmatched']} ({match_stats['unmatched'] / total_csv_rows * 100:.1f}%)"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Merge CSV with Excel/CSV data containing technical equipment information."
    )

    # Add mandatory arguments
    parser.add_argument(
        "--csv", required=True, help="Path to the CSV file with company keywords data"
    )
    parser.add_argument(
        "--base",
        required=True,
        help="Path to the base data file (CSV or Excel) with technical equipment information",
    )
    parser.add_argument(
        "--output",
        required=False,
        help="Path where the merged output file will be saved. If not provided, a name will be generated based on the input CSV",
    )
    parser.add_argument(
        "--log-level",
