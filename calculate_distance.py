...
OUTPUT_COLUMNS = [COL_COMPANY, COL_URL]  # Configurable - add or remove columns as needed

def clean_address(street: Optional[str], plz: Optional[Union[str, int]], city: Optional[str]) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    """
    ...
    if cleaned_street:
        ...
        street_number_match = re.search(r'(\d+)(?:[a-z])?(?:-\d+[a-z]?)?(?:,\s*\d+[a-z]?)*\s*$', cleaned_street)
        if street_number_match:
            base_number = street_number_match.group(1)
            # Replace the complex number with just the first number
            cleaned_street = re.sub(r'(\d+)(?:[a-z])?(?:-\d+[a-z]?)?(?:,\s*\d+[a-z]?)*\s*$', base_number, cleaned_street)
    ...
    if cleaned_plz:
        ...
        if len(cleaned_plz) != 5 or not cleaned_plz.isdigit():
            logging.warning(f"Invalid postal code: {cleaned_plz}, will be omitted from geocoding query")
            cleaned_plz = None
...
def geocode_address(address: str, geolocator: Nominatim, retries: int = 3, 
                 fallback_to_city: bool = True, 
                 parsed_plz: Optional[str] = None, 
                 parsed_city: Optional[str] = None) -> Optional[Tuple[float, float]]:
    """
    ...
    for attempt in range(retries):
        try:
            ...
            if location:
                ...
                logging.warning(f"Could not geocode address after {retries} attempts: {address}")
                
                # Try fallback to just PLZ and city if specified
                if fallback_to_city and parsed_plz and parsed_city:
                    logging.info(f"Trying fallback geocoding with just PLZ and city for: {address}")
                    ...
                    try:
                        ...
                        if fallback_location:
                            logging.info(f"✓ Fallback geocoding successful for: {address}")
                            logging.info(f"  Using approximate coordinates from: {fallback_address}")
            ...
            logging.warning(f"Geocoding error for {address}: {str(e)}. Attempt {attempt+1}/{retries}")
...
def calculate_distances(input_file: str, reference_address: str, output_file: Optional[str] = None, testing: bool = False) -> bool:
    """
    ...
    try:
        ...
        if missing_columns:
            logging.error(f"Required columns missing from input file: {', '.join(missing_columns)}")
            logging.error(f"Available columns: {', '.join(df.columns)}")
            logging.error("Please check column names in configuration constants at the top of the script")
            return False
    ...
    processed_companies: Dict[str, Tuple[Optional[float], Optional[float]]] = {}  # Dictionary to store company name -> (lat, lon) mapping
    existing_df = None
    if os.path.exists(output_file):
        try:
            existing_df = pd.read_csv(output_file, encoding='utf-8-sig')
            if COL_COMPANY in existing_df.columns and 'latitude' in existing_df.columns and 'longitude' in existing_df.columns:
                ...
                logging.info(f"Found {len(processed_companies)} previously processed companies")
        except Exception as e:
    ...
    if processed_companies:
        ...
        df_filtered = df[~df[COL_COMPANY].apply(lambda x: 
            x in processed_companies and 
            not (pd.isna(processed_companies[x][0]) or pd.isna(processed_companies[x][1]))
        )]
        ...
        retry_count = sum(1 for company in df[COL_COMPANY] if 
                         company in processed_companies and 
                         (pd.isna(processed_companies[company][0]) or pd.isna(processed_companies[company][1])))
        
        logging.info(f"Skipping {skipped_count} already processed companies with valid coordinates")
        logging.info(f"Will retry {retry_count} companies with failed geocoding")
        
        if len(df) == 0:
            logging.info("All companies have already been processed successfully. Nothing to do.")
            return True
    ...
    df['full_address'] = df.apply(
        lambda row: f"{row[COL_STREET]}, {row[COL_POSTAL]} {row[COL_CITY]}", 
        axis=1
    )
    ...
    include_header = not os.path.exists(output_file) or os.path.getsize(output_file) == 0
    
    # Process each company and write incrementally
    for idx, row in df.iterrows():
        company_name = row[COL_COMPANY]
        address = row['full_address']
        logging.info(f"Processing company {idx+1}/{len(df)}: {company_name}, {address}")
        
        # Clean address components
        cleaned_street, cleaned_plz, cleaned_city = clean_address(row[COL_STREET], row[COL_POSTAL], row[COL_CITY])
        ...
        if existing_df is not None:
            ...
            if company_exists:
                ...
                existing_df.loc[existing_df[COL_COMPANY] == company_name] = result_df.values[0]
...
def calculate_single_distance(address: str, reference_address: str) -> Optional[Tuple[float, float, float]]:
    """
    ...
    coords = geocode_address(clean_address_str, geolocator, 
                           fallback_to_city=True,
                           parsed_plz=cleaned_plz, 
                           parsed_city=cleaned_city)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Calculate distances from addresses to a reference point.')
    parser.add_argument('--file', '-f', type=str, help='Input Excel/CSV file with addresses')
    parser.add_argument('--reference', '-r', type=str, default="Hollerithallee 17, 30419 Hannover",
                        help='Reference address to calculate distances from')
    parser.add_argument('--address', '-a', type=str, help='Single address to check (will not save to file)')
    parser.add_argument('--output', '-o', type=str, help='Output file path (optional)')
    parser.add_argument('--test', '-t', action='store_true', help='Testing mode - only process first 5 addresses')
    ...
    # Process a file of addresses
        ...
        calculate_distances(INPUT_FILE, REFERENCE_ADDRESS, OUTPUT_FILE, testing=TESTING_MODE)
